...
export class KeycloakPermissionsHandler {
  ...
  constructor(
    private keycloakMultiRealm: KeycloakMultiRealm,
    token: Token | undefined,
    private config: AuthorizationConfiguration | undefined,
    userRequest?: any
  ) {
  ...
  private handlePermissions(
    permissions: string[],
    handler: (r: string, s: string | undefined) => boolean
  ) {
  ...
  async hasPermission(resources: string | string[]): Promise<boolean> {
    ...
    if (
      this.handlePermissions(expectedPermissions, (resource, scope) => {
        if (
          this.permissionsToken &&
          this.permissionsToken.hasPermission(resource, scope)
        ) {
          return true;
        }
        return false;
    ...
    let authzRequest: AuthZRequest = {
      audience:
        this.config?.resource_server_id ??
        this.keycloakMultiRealm
          .getKeycloakObjectFromReq(this.userRequest)
          .getConfig().resource,
    ...
    try {
      ...
      const grant = await this.keycloakMultiRealm
        .getKeycloakObjectFromReq(this.userRequest)
        .checkPermissions(authzRequest, this.req);
