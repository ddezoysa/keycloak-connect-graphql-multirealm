...
import { KeycloakMultiRealm } from './multi-realm/KeycloakMultiRealm';
import { Request } from 'express';
import {
  AuthorizationConfiguration,
  KeycloakPermissionsHandler,
} from './KeycloakPermissionsHandler';
import { getDefaultRealmNameFromTenant } from './directives/directiveResolvers';
...
export class KeycloakContextBase implements AuthContextProvider {
  ...
  public readonly accessToken: Keycloak.Token | undefined | any;
  public static contextKey = CONTEXT_KEY;
  req: any;
  constructor(
    token?: Keycloak.Token,
    private keycloak: KeycloakMultiRealm | undefined = undefined,
    authorizationConfiguration:
      | AuthorizationConfiguration
      | undefined = undefined,
    req?: any | undefined
    ...
    this.req = req;
    if (keycloak) {
      this.permissionsHandler = new KeycloakPermissionsHandler(
        keycloak,
        token,
        authorizationConfiguration,
        req
  ...
  public tenant(tenantName: string): boolean {
    ...
    if (
      ...
      this.accessToken.content.iss &&
      this.accessToken.content.iss.startsWith(
        this.keycloak?.getKeycloakConfig()['auth-server-url']
      )
    ) {
      const tokenRealm = this.accessToken.content.iss.split('/').pop();
      return tokenRealm == this.keycloak?.realmTenantMappingFn(tenantName);
    } else return false;
  }
}
  ...
  implements AuthContextProvider
{
  ...
  constructor(
    ...
    keycloak: KeycloakMultiRealm | undefined = undefined,
    authorizationConfiguration:
      | AuthorizationConfiguration
      | undefined = undefined
  ) {
